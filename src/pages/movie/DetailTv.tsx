...
import type {Cast, Video, Movie } from '../../types/index'
...
const DetailTv: React.FC = () => {
  ...
  useEffect(() => {
    AxiosInstance.get(`/tv/${id}`, { params: { api_key: import.meta.env.VITE_TMBD_KEY } })
      .then((res) => { setIsLoading(false); dispatch(setDataFilmById(res.data)) })
  }, [id, dispatch])

  useEffect(() => {
    AxiosInstance.get(`/tv/${id}/credits`, { params: { api_key: import.meta.env.VITE_TMBD_KEY } })
      .then((res) => setDataCredits(res.data.cast))
  }, [id])

  useEffect(() => {
    AxiosInstance.get(`/tv/${id}/recommendations`, { params: { api_key: import.meta.env.VITE_TMBD_KEY } })
      .then((res) => setDataRecommendations(res.data.results))
  }, [id])

  useEffect(() => {
    AxiosInstance.get(`/tv/${id}/videos`, { params: { api_key: import.meta.env.VITE_TMBD_KEY } })
      ...
      <div className="detail__container" style={{ backgroundImage: `url(${import.meta.env.VITE_IMG_URL}/${dataFilm.backdrop_path})` }}>
        ...
        <div className="container data__detail d-flex gap-3">
          ...
          <div className="content__detail d-flex flex-wrap gap-2 flex-column">
            <span className="fs-5">{dataFilm.title || dataFilm.name}</span>
      ...
      <section className="content__detail__container font__color container">
        ...
        <section className="my-3">
          ...
          <div className="cards__recommendation">
            <SliderFilm>
              {dataRecommendations.map((rec, i) => <Link to={`/detail-tv/${rec.id}`} key={i} className="Link"><CardFilm data={rec} /></Link>)}
            </SliderFilm>
          </div>
        </section>
      </section>
